...
poll:
	...
	mov rsi, command_load_inline
	call string_compare		; comps rsi to rdi(input str)
	jc load
...
load:
	...
	jmp poll
load_inline:
	; call input ; TODO hatt - get number from input as 1 line
	call string_to_int
	sub rax, 1			; Files are indexed from 0
	push rax			; Save the file #
	; check value
...
command_uname:		db 'uname', 0
command_load:		db 'load', 0
command_load_inline:	db 'load ', 0
command_help:		db 'help', 0
cpumsg:			db '[cpu: ', 0
osname:			db 'BareNakedOS',0
memmsg:			db ']  [mem: ', 0
networkmsg:		db ']  [net: ', 0
diskmsg:		db ']  [hdd: ', 0
mibmsg:			db ' MiB', 0
...
screen_clear:
	...
	mov rdi, [VideoBase]
	xor eax, eax
	mov al, [BG_Color]		; TODO - needs to use the whole value, currently uses last byte
	mov ecx, [Screen_Bytes]
	add ecx, 300000			; Fudge value for last line.. gross
	rep stosb
